...
const YogaList = () => {
  ...
  const filteredYoga = yogaData.filter(item => {
    return (
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="md:col-span-2">
          <Label htmlFor="search" className="text-sm mb-2 block">Search Yoga Practices</Label>
          <Input
            id="search"
      ...
      {filteredYoga.length === 0 ? (
        <div className="text-center py-8">
          <p className="text-gray-500">No yoga practices match your search criteria.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredYoga.map((item: YogaItem) => (
            <YogaCard key={item.id} item={item} />
          ))}
        </div>
      )}
    </div>
